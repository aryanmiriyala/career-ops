...
function Jobs({ onIntake }: { onIntake: (job: Job) => void }) {
  ...
  const [selected, setSelected] = useState<Job>(); const [error, setError] = useState(''); const [loading, setLoading] = useState(false); const [revision, setRevision] = useState(0); const [limit, setLimit] = useState('20');
  ...
  return <><header className="page-heading"><div><div className="eyebrow">DISCOVERY</div><h1>Job board</h1><p className="muted">Last seen {date(board?.last_seen_at)}</p></div><div className="scan-actions"><label className="sr-only" htmlFor="scan-limit">Scan scope</label><select id="scan-limit" value={limit} disabled={running} onChange={e => setLimit(e.target.value)}><option value="20">20 boards / source</option><option value="5">5 boards / source</option><option value="0">All boards</option></select><button className="primary" disabled={running} onClick={async () => { try { setScan(await api<Scan>('/scans', { company_limit: Number(limit) })); } catch (e) { setError((e as Error).message); } }}>{running ? <Busy/> : <RefreshCw size={16}/>}Refresh postings</button></div></header>
    <div className="metrics"><div><span>In this view</span><strong>{board?.total ?? '—'}</strong></div><div><span>Stored postings</span><strong>{board?.stored ?? '—'}</strong></div><div><span>Unknown posted date</span><strong>{board?.undated ?? '—'}</strong></div><div><span>Discovery model calls</span><strong>0<span className="metric-unit"> / scan</span></strong></div></div>
    ...
    <div className="view-tabs">{['all', 'shortlist', 'review', 'blocked'].map(s => <button key={s} className={scope === s ? 'active' : ''} onClick={() => { setScope(s); setPage(1); }}>{s === 'all' ? 'All matches' : label(s)}</button>)}{loading && <Busy/>}</div>
    <div className="board-content"><div className="table-wrap"><table><thead><tr><th>Role / company</th><th>Location</th><th>{basis === 'posted' ? 'Posted' : 'First discovered'}</th><th>Review status</th><th><span className="sr-only">Details</span></th></tr></thead><tbody>{board?.jobs.map(job => <tr key={job.id} className={selected?.id === job.id ? 'selected-row' : ''}><td><button className="job-title" onClick={() => setSelected(job)}>{job.title}</button><div className="company-line"><span className="company-monogram" aria-hidden="true">{job.company.slice(0, 1)}</span>{job.company}<span className="source-tag">{job.source}</span></div></td><td>{job.location}</td><td className="date-cell">{date(basis === 'posted' ? job.posted_at : job.first_seen_at)}</td><td><Badge value={job.status}/></td><td><button className="icon" title="View job" aria-label={'View ' + job.title} onClick={() => setSelected(job)}><ChevronRight size={18}/></button></td></tr>)}</tbody></table>
    ...
    {selected && <Dialog sheet onClose={() => setSelected(undefined)}><div className="mb-6 pr-9"><Badge value={selected.status}/></div><div className="eyebrow">{selected.company}</div><DialogTitle className="mb-4 text-[22px] font-semibold leading-7">{selected.title}</DialogTitle><p className="location"><MapPin size={16}/>{selected.location}</p><dl><dt>Posted</dt><dd>{date(selected.posted_at)}</dd><dt>First discovered</dt><dd>{date(selected.first_seen_at)}</dd><dt>Source</dt><dd>{selected.source}</dd><dt>Discovery fit estimate</dt><dd>{selected.score == null ? 'Not recorded' : `${selected.score}/100`}</dd></dl><div className="flag-list">{selected.flags.map(f => <Badge key={f} value={f}/>)}</div>{selected.notes && <p className="muted">{selected.notes}</p>}<div className="drawer-actions"><Button asChild><a href={selected.url} target="_blank" rel="noopener noreferrer">Open posting<ExternalLink size={16}/></a></Button><Button variant="primary" onClick={() => { onIntake(selected); setSelected(undefined); }}><FileText size={16}/>Prepare application</Button></div></Dialog>}
  </>;
}
...
function App() {
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand"><span className="brand-icon"><BriefcaseBusiness size={21}/></span>Career Ops</div><div className="sidebar-label">WORKSPACE</div><nav>{[{ id: 'jobs', title: 'Job board', icon: Search }, { id: 'applications', title: 'Applications', icon: FileText }, { id: 'providers', title: 'Providers', icon: KeyRound }].map(({ id, title, icon: Icon }) => <button key={id} className={view === id ? 'active' : ''} onClick={() => setView(id)}><Icon size={18}/>{title}{view === id && <ChevronRight size={14}/>}</button>)}</nav><div className="sidebar-bottom"><div className="local-indicator"><span className="dot configured"/>Local instance</div><div className="account"><span title={session.email}>{session.email}</span><button className="icon" title="Sign out" aria-label="Sign out" onClick={async () => { try { await api('/auth/logout', {}); } finally { await supabase?.auth.signOut(); setSession({ authenticated: false }); } }}><LogOut size={17}/></button></div></div></aside>
    <main className="main"><div className="topbar"><span><BriefcaseBusiness size={14}/>Personal workspace<ChevronRight size={13}/>{view === 'jobs' ? 'Job board' : view === 'applications' ? 'Applications' : 'Providers'}</span><span><Clock3 size={13}/>{new Date().toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}</span></div><div className="page">{view === 'jobs' ? <Jobs onIntake={job => setModal({ job })}/> : view === 'applications' ? <Applications current={current} onSelect={setCurrent} onNew={() => setModal({})}/> : <Providers/>}</div></main>
