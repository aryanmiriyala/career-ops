...
Use this file to assemble targeted resumes. Bullets should be truthful, ATS-friendly, and selected based on the job description.

## Bullet Formula

Action verb + what was built/changed + technical keywords + scope/domain + impact.

Good pattern:

- Built X using Y to support Z.
- Automated X with Y, processing Z scale.
- Secured X by implementing Y across Z workflows.
- Modernized X from legacy Y to standards-compliant Z.

Avoid:

- Vague claims without technology.
- Unsupported metrics.
...
- Orchestrated AWS Glue Workflows to automate ETL pipelines for billing, MDM migration, reverse ETL, and data validation.
...
## Cybersecurity Education / Graduate Research Assistant

Use these mainly for LinkedIn, cover letters, education/cybersecurity roles, or a broader experience section. For a one-page software resume, use only when the job description values cybersecurity, technical training, curriculum design, or security lab work.
...
- Modernized legacy ColdFusion/Lucee forms with HTML5 date/time pickers, themed CSS, JavaScript fallbacks, and cfform script updates to improve standards compliance, data entry, and graceful degradation.
- Refactored MatchRun, registration, offer, and combination interfaces into reusable modal/tab components with icons and tooltips to improve usability and maintainability for internal transplant operations.
...
- Replaced legacy Pentaho jobs with AWS Glue Workflows and partitioned S3 pipelines for modern MDM ingestion.
