...
11. Create a tailored `resume.tex` from `master-documents/master-resume/resume.tex` or an approved canonical resume template. Do not read previous application packages to discover the latest visual pattern during a normal new-package run.
...
15. Run a resume-vs-job-description alignment pass after generating the resume. Include a Job Alignment & Evidence Score, matched keywords, missing-but-truthful keyword opportunities, unsupported keywords intentionally omitted, and concrete next-step recommendations.
...
- Use `profile/`, `master-documents/`, project repositories, the saved job description, and canonical templates as the default source set.
...
- **Visual Consistency Gate**: Before compiling a new application resume, compare its LaTeX preamble and layout macros against the canonical application-resume visual system in `master-documents/master-resume/resume.tex` or an approved canonical template. Reuse the same document class, geometry, font, header structure, section styling, bullet label, role/date hierarchy, and skills formatting. Tailoring should change evidence and ordering, not visual identity.
