...
- `Exact posted title matched in visible summary/title/header clause`
- `Strong matches`
...
11. Confirm the Professional Summary, when used, renders as no more than two PDF text lines. Rewrite or omit it if it renders as three lines or repeats skills already visible below.
12. Confirm bullets are recruiter-readable without internal ticket/tool context and that private-source details have been translated into public-safe product/system language.
13. Confirm the Experience section normally contains at least 11 strong bullets; if it does not, add verified role-aligned evidence or document an `Experience Bullet Count Waiver`.
14. Confirm `tailoring-notes.md` records `Pass` for `ATS source gate checked`, `Visual consistency gate checked`, `Human recruiter readability gate checked`, `Submitted-facing terminology sync checked`, `Score consistency gate checked`, `Cover-letter artifact checked`, and `Pass` or `Waived - <reason>` for `Page utilization gate checked`.
15. Run `python3 automation/validate_application_package.py application-packages/<Company>/<Role>` and address any failures.
16. Report changed files, verification results, validator result, and the Job Alignment & Evidence Score with the disclaimer that it is not a predicted ATS result.
17. Remove generated LaTeX build artifacts. Keep submission PDFs only when Aryan asks for final application artifacts or when the application package needs a ready-to-submit PDF.
