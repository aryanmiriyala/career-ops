...
- Treat the saved full job description as the preserved source of truth, but after the first keyword extraction pass, work primarily from the compact job keyword map, Gap Recovery Gate notes, analyzer output, and targeted `rg`/`sed` reads. Do not repeatedly paste or reopen the full posting unless a new requirement, blocker, or wording question requires it.
- Do not print raw full outputs from `pdftotext`, `pdftotext -bbox`, broad `find application-packages`, full tracker reads, or full profile master files during routine validation. Use `automation/validate_application_package.py`, `automation/analyze_application_keywords.py`, `automation/audit_token_hotspots.py`, `rg`, and small line-range reads to get bounded summaries while preserving the same quality gates.
- For expanded role-specific keyword audits with many terms, prefer a package-local term list file and `python3 automation/analyze_application_keywords.py <package> --term-file <file>` over long repeated `--term` commands. Record the resulting found/missing summary in `tailoring-notes.md`.
...
5. Confirm extractable text with `pdftotext resume.pdf -` through the validator or a targeted local inspection; do not paste the full extracted resume text into the conversation unless diagnosing a parser-order failure. Contact details, headings, dates, roles, and bullets must still appear in the intended reading order.
6. If `pdftotext -bbox resume.pdf -` is available, use the validator's bottom-page summary or a bounded local check to confirm the unused area is only a small finishing margin, not merely below the validator's hard failure threshold. If verified role-aligned evidence can fit cleanly, close the gap before marking page utilization as passed. A substantially underfilled page fails unless `tailoring-notes.md` explains that no additional verified role-aligned evidence should be added.
