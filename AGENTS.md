...
If a profile guide, template, automation script, prior application, or reusable prompt conflicts with this file, this file takes precedence. Update the conflicting dependency before using it to finalize a new application package; do not silently fall back to the older rule. Do not inspect prior application packages just to look for conflicts; use targeted search only after a specific conflict or reusable source is identified.
...
4. Review `profile/evidence-index.md` first for relevant experience, projects, skills, reusable bullets, lane routing, and common unsupported claims. Open full profile master files only for exact wording, source verification, or gaps not covered by the index.
...
7. Run the **Gap Recovery Gate** before final bullet/project selection. For every important job requirement, preferred skill, tool, domain term, or responsibility that is not already planned for the resume, classify it as one of:
   - `Supported - add/use now`: verified in `profile/`, `master-documents/`, project repositories, another inspected source, or a specifically relevant prior application note, and should be added to the resume, cover letter, skills section, or tailoring notes if it improves fit.
   ...
   Record the gap source, recovery decision, where it was added or why it was not added, and any source-material update needed in `tailoring-notes.md`. This gate exists so older projects, hackathons, coursework, private repos, and partially documented builds are not accidentally ignored, while still keeping submitted claims interview-defensible.
...
9. Apply `profile/cover-letter-guide.md` before drafting any cover letter. Use known personal context from `profile/` and targeted user-provided context. Prior application notes should be opened only when they are specifically relevant to the company, domain, prompt, referral, or reusable answer. Ask Aryan cover-letter personalization questions only when the letter would be materially weaker or risky without the answer:
...
10. Document the resume direction, Target Professional Title Clause, Gap Recovery Gate result, F-1 Work Authorization Gate result, referral plan, cover-letter angle, strongest matching experience/projects, important keyword targets, unsupported keywords to avoid, and any blocking eligibility questions in `tailoring-notes.md`.
...
12. Audit every experience and project bullet against the bullet rules before compiling. Rewrite any bullet that lacks a strong action verb, a specific contribution, truthful method or technology when relevant, scope/domain context, and impact/result.
...
15. Run a resume-vs-job-description alignment pass after generating the resume. Use `python3 automation/analyze_application_keywords.py application-packages/<Company>/<Role>` when a resume artifact exists, then include a Job Alignment & Evidence Score, matched keywords, missing-but-truthful keyword opportunities, unsupported keywords intentionally omitted, and concrete next-step recommendations.
16. Add `tailoring-notes.md` explaining which experience, projects, and keywords were emphasized, plus the Gap Recovery Gate, F-1 Work Authorization Gate, bullet audit, ATS source gate, visual consistency gate, page utilization gate, alignment pass, and Job Alignment & Evidence Score.
...
- Use `profile/evidence-index.md`, targeted `profile/` source sections, `master-documents/`, project repositories, the saved job description, and canonical templates as the default source set.
...
- Use targeted `rg` searches for exact job-description keywords, unsupported gaps, or specific company/domain context instead of broad historical-package scans.
- Open a prior application package only when Aryan names it, when a specific reusable answer/evidence item is known to exist there, or when resolving a concrete conflict that cannot be handled from current source materials.
...
- AI-assisted bullets must be manually edited until they are specific, grounded, and defensible. Reject bullets that are grammatically polished but vague, inflated, buzzword-heavy, emotionally generic, or not traceable to `profile/`, `master-documents/`, project repositories, the saved job description, or a specifically relevant prior application note.
...
- Use verified personal context already in `profile/` and targeted user-provided context. Open prior application notes only when they are specifically relevant to the company, domain, prompt, referral, or reusable answer. Ask Aryan about personal connection, motivation, or desired impression only when the letter would be materially weaker or risky without the answer.
...
5. If `pdftotext -bbox resume.pdf -` is available, confirm the bottom unused area is only a small finishing margin, not merely below the validator's hard failure threshold. If verified role-aligned evidence can fit cleanly, close the gap before marking page utilization as passed. A substantially underfilled page fails unless `tailoring-notes.md` explains that no additional verified role-aligned evidence should be added.
6. Render the resume and inspect it against the canonical application-resume visual system for border consistency, name/contact placement, section styling, bullet appearance, role/date hierarchy, density, bottom-page usage, clipping, and overlap. A visibly inconsistent, sparse, or fillably underused bottom area fails this check even when the automated validator passes.
7. Run `python3 automation/analyze_application_keywords.py application-packages/<Company>/<Role>` and use the exact-term report to identify missing truthful keyword opportunities and unsupported keywords that should stay omitted.
8. Run the resume-vs-job-description alignment pass and record the Job Alignment & Evidence Score, score methodology, point-by-point breakdown, and its internal-estimate disclaimer in `tailoring-notes.md`.
9. Confirm mandatory qualifications and the highest-priority truthful keywords appear in evidence-bearing context; record unsupported requirements as gaps rather than adding them.
10. Confirm the employer's exact posted role title appears once in visible resume text through a truthful Target Professional Title Clause, and that the title is supported by nearby experience or project evidence rather than repeated as a disconnected keyword.
11. Confirm the Professional Summary, when used, renders as no more than two PDF text lines. Rewrite or omit it if it renders as three lines.
12. Confirm the Experience section normally contains at least 11 strong bullets; if it does not, add verified role-aligned evidence or document an `Experience Bullet Count Waiver`.
13. Confirm `tailoring-notes.md` records `Pass` for `ATS source gate checked`, `Visual consistency gate checked`, `Cover-letter artifact checked`, and `Pass` or `Waived - <reason>` for `Page utilization gate checked`.
14. Run `python3 automation/validate_application_package.py application-packages/<Company>/<Role>` and address any failures.
15. Report changed files, verification results, validator result, and the Job Alignment & Evidence Score with the disclaimer that it is not a predicted ATS result.
16. Remove generated LaTeX build artifacts. Keep submission PDFs only when Aryan asks for final application artifacts or when the application package needs a ready-to-submit PDF.
